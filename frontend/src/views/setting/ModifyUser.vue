    ...
    <div class="inside">
      <div class="modify-user">
        ...
        <v-card-text class="text-input">
          <v-flex class="mb-4 text-center">
            ...
            <div v-else>
              <image-input v-model="avatar">
                <div slot="activator">
                  <v-avatar size="128px" v-ripple v-if="!avatar" class="grey lighten-3 mb-3">
                    <span>Click</span>
                  </v-avatar>
              ...
              <v-slide-x-transition>
                <div v-if="avatar && saved == false">
                  <v-btn class="primary" @click="uploadImage" :loading="saving">저장</v-btn>
                </div>
              </v-slide-x-transition>
            </div>
          </v-flex>
          <v-text-field disabled v-model="userName" label="이름"></v-text-field>
          <v-text-field disabled v-model="userEmail" label="이메일" type="email"></v-text-field>
          <v-text-field
          ...
          <button class="btn-get-check" type="button" @click="checkNickname">중복확인</button>
          <v-text-field
...
import { required, minLength, sameAs, helpers, numeric, maxLength } from "vuelidate/lib/validators";
...
export default {
  ...
  methods: {
    ...
    uploadImage() {
      ...
      this.userImage = fd;
      this.$Axios
        .post(`${this.$store.state.host}/v1/users/` + this.userEmail + `/image`, this.userImage, {
          headers: {
